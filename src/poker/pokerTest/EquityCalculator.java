    ...
    public void calculate() throws Exception {
        ...
        if (boardCount == 0) {
            ...
            for (long iterations = mMaxIterations; iterations > 0; iterations--) {
                ...
                List<Integer> highestRankingHand = new ArrayList<>();
                for (int z = 0; z < handsCount; z++) {
                    ...
                    if (highestRanking == null || hr.compareTo(highestRanking) > 0) {
                        highestRanking = hr;
                        highestRankingHand.clear();
                        highestRankingHand.add(z);
                    } else if (hr.compareTo(highestRanking) == 0) {
                        highestRankingHand.add(z);
                    }
                }
                if (highestRankingHand.size() > 1) {
                    for (int index : highestRankingHand) {
                        mEquities.get(index).playersGotTie(true);
                    }
                } else {
                    for (int index : highestRankingHand) {
                        mEquities.get(index).addPossibleHand(true);
                    }
                }

                for (int z = 0; z < handsCount; z++) {
                    if (!highestRankingHand.contains(z)) {
                        mEquities.get(z).addPossibleHand(false);
                    }
                }
            }
